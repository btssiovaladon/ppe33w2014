...
if(isset($_POST['Modifier'])){
	$resultats = $co->prepare("UPDATE `action` SET `N_ACTION`=:action,`N_AMIS`=:amis,`DATE_DEB_ACTION`=:debaction,`DUREE_ACTION`=:duree,`FONDS_COLLECTES`=:fonds WHERE N_ACTION=1;");
	$resultats->execute(array(':action' => $_POST['action'], 
							':amis' => $_POST['amis'], 
